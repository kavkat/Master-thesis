...
pluri.axd1 <- read.csv('axd1.pluri.csv')
str(pluri.axd1)
samples <- c(rep('axd1',3), rep('h1',3))
mat.data <- cbind(samples, pluri.axd1[,2], pluri.axd1[,3], pluri.axd1[,4])
...
bp<-barplot(mat.mean, beside=TRUE, ylim=c(0,0.12), xlab='Genes analysed', ylab='Expression normalized to GadPH', main='Pluripotency analysis of LU AxD1', col=c('lightseagreen', 'lightcoral'), legend.text =c('LU AxD1', 'H1 positive control'))
